...
public abstract class ExtrinsicEvaluation extends Evaluation{
	 ...
	 */
	protected HashMap<String, HashMap<String, Integer>> createConfusionMatrix(String classDir, String clusterDir){
		...
		if (listClass.length>0 && listClusters.length>0){
			...
			throw new IllegalArgumentException("Error : No class directories in "+classDir);
		} else {
			throw new IllegalArgumentException("Error : No cluster directories in "+clusterDir);
		}
	 ...
	 * @return HashMap<String cluster, HashMap<String class, Integer intersection>>
	 */
	protected HashMap<String, HashMap<String, Integer>> createConfusionMatrix(String classDir, ArrayList<Cluster> clusterList){
		...
		if (listClass.size()>0 && clusterList.size()>0){
			...
			throw new IllegalArgumentException("Error : There's no files in "+classDir);
		} else {
			throw new IllegalArgumentException("Error : There's no cluster ");
		}
